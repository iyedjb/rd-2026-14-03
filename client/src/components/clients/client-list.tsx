...
export function ClientList({
  ...
  const getStatusBadge = (client: Client) => {
    ...
    } else if (typeof travelDate === 'string' || typeof travelDate === 'number') {
      travelDateObj = new Date(travelDate);
    } else if (typeof travelDate === 'object' && travelDate !== null) {
      const seconds = (travelDate as any)._seconds ?? (travelDate as any).seconds;
      if (typeof seconds === 'number') {
        travelDateObj = new Date(seconds * 1000);
      } else {
        travelDateObj = new Date(NaN);
      }
    } else {
      travelDateObj = new Date(NaN);
    }
