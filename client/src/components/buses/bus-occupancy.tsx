...
import { generateHotelPDF, generateListaCompletaPDF, generateBusGraphicPDF } from "@/lib/pdf-generator";
...
export function BusOccupancy() {
  ...
  const [showHotelRoomModal, setShowHotelRoomModal] = useState(false);
  const [hotelRoomSelection, setHotelRoomSelection] = useState<any[]>([]);
  const [hotelRoomLinkLoading, setHotelRoomLinkLoading] = useState(false);
  ...
  const assignExistingClientMutation = useMutation({
    mutationFn: async (data: { client_id: string; destination_id: string; bus_id: string; seat_number: string; child_id?: string; passenger_type?: string }) => {
  ...
  const handleOpenPassengerModal = (pdfType: "embarque" | "motorista" | "hotel" | "lista_completa") => {
    ...
    if (pdfType === "hotel") {
      ...
      setShowHotelRoomModal(true);
    } else {
      setShowPassengerModal(true);
    }
  };

  const hotelFamilies = useMemo(() => {
    const groups = new Map<string, any[]>();
    for (const p of reservationsWithClients || []) {
      const groupingId = p.parent_client_id || p.client_id;
      if (!groups.has(groupingId)) groups.set(groupingId, []);
      groups.get(groupingId)?.push(p);
    }
    return Array.from(groups.entries()).map(([key, members]) => ({ key, members }));
  }, [reservationsWithClients]);

  const getHotelRoomTypeForUI = (familyMembers: any[]) => {
    const familySize = familyMembers.length;
    const hasSpouse = familyMembers.some(
      (m: any) => m.is_child && m.child_data?.relationship === "cônjuge"
    );
    const hasChildren = familyMembers.some((m: any) => {
      const rel = m.child_data?.relationship;
      return (
        m._isOverrideChild ||
        (m.is_child && (rel === "filho(a)" || rel === "filho" || rel === "filha"))
      );
    });

    if (familySize === 1) return "SINGLE";
    if (familySize === 2) return hasSpouse ? "CASAL" : "DUPLO SOLTEIRO";
    if (familySize === 3) {
  ...
  const hotelSingleFamilies = useMemo(() => {
    const singles = hotelFamilies
      .map((f) => ({ ...f, roomType: getHotelRoomTypeForUI(f.members) }))
      .filter((f) => f.roomType === "SINGLE" && f.members.length === 1 && !f.members[0]?.is_child);
  ...
  const hotelGroupedFamiliesForUI = useMemo(() => {
    return hotelFamilies
      .map((f) => ({ ...f, roomType: getHotelRoomTypeForUI(f.members) }))
      .filter((f) => f.roomType !== "SINGLE")
      .sort((a, b) => (a.members.find((m: any) => !m.is_child)?.client_name || "").localeCompare(b.members.find((m: any) => !m.is_child)?.client_name || ""));
  }, [hotelFamilies]);

  const hotelDuploSolteiroRoomsForUI = useMemo(() => {
    return hotelGroupedFamiliesForUI.filter((f) => f.roomType === "DUPLO SOLTEIRO");
  }, [hotelGroupedFamiliesForUI]);

  const toggleHotelRoomSelection = (passenger: any) => {
    setHotelRoomSelection((prev) => {
  ...
  const getFamilyMainName = (familyMembers: any[]) => {
    return (familyMembers.find((m: any) => !m.is_child) || familyMembers[0])?.client_name;
  };

  const selectedHotelSingle = hotelRoomSelection.length === 1 ? hotelRoomSelection[0] : null;

  const handleSaveHotelRoomsSelection = async () => {
    if (!selectedDestination || hotelRoomSelection.length < 2) return;

    const selectionCount = hotelRoomSelection.length;
    if (selectionCount !== 2 && selectionCount !== 3) return;

    setHotelRoomLinkLoading(true);
    try {
      const main = hotelRoomSelection[0].client_name;
      const companions = hotelRoomSelection.slice(1).map((p) => p.client_name);
      const type = selectionCount === 2 ? "DUPLO_SOLTEIRO" : "TRIPLO_SOLTEIRO";

      const response = await fetch("/api/fix-hotel-rooms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          destination: selectedDestination.name,
          groups: [{ main, companions, type }],
        }),
      });
      ...
      toast({
        title: "Quarto de solteiro salvo!",
        description:
          selectionCount === 2
            ? "Duplo solteiro gerado e salvo para o PDF Hotel."
            : "Triplo solteiro gerado e salvo para o PDF Hotel.",
      });

      setHotelRoomSelection([]);

      // Refresh reservations/guests so the popup reflects what was saved
      ...
      toast({
        ...
        description: err?.message || "Não foi possível salvar o quarto. Tente novamente.",
        variant: "destructive",
      });
    } finally {
      setHotelRoomLinkLoading(false);
    }
  };

  const handleAddSingleToDuploRoom = async (duploMainClientName: string) => {
    if (!selectedDestination || !selectedHotelSingle) return;
    if (!duploMainClientName) return;

    // Duplo -> Triplo (sempre solteiro)
    setHotelRoomLinkLoading(true);
    try {
      const addedSingleName = selectedHotelSingle.client_name;
      ...
      const response = await fetch("/api/fix-hotel-rooms", {
        ...
        body: JSON.stringify({
          destination: selectedDestination.name,
          groups: [{ main: duploMainClientName, companions: [addedSingleName], type: "TRIPLO_SOLTEIRO" }],
        }),
      });
      ...
      toast({
        ...
        description: "O passageiro foi adicionado ao quarto e já refletirá no PDF Hotel.",
      });
      ...
      toast({
        title: "Erro ao adicionar ao quarto",
        description: err?.message || "Não foi possível adicionar ao quarto. Tente novamente.",
        variant: "destructive",
      });
    } finally {
      setHotelRoomLinkLoading(false);
  ...
  const handleContinueToHotelPDFReview = () => {
    setShowHotelRoomModal(false);
    setShowPassengerModal(true);
  };
    ...
    <div className="space-y-1">
                ...
                <Button
                  onClick={() => {
                    if (selectedBus && selectedDestination) {
                      const seatInfo: { [key: string]: string } = {};
                      reservationsWithClients.forEach((r: any) => {
                        if (r.seat_number && r.client_name) {
                          seatInfo[r.seat_number] = r.client_name;
                        }
                      });
                      generateBusGraphicPDF(
                        selectedBus.name,
                        selectedBus.type || '',
                      ...
                      toast({ title: "PDF gerado!", description: "Gráfico do ônibus salvo com sucesso." });
                    }
                  }}
                  variant="outline"
                  size="sm"
                  data-testid="button-generate-bus-graphic-pdf"
                >
                  <BusIcon className="h-4 w-4 mr-2" />
                  Gráfico Ônibus
                </Button>
      ...
      >
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              Hotel - Agrupar Quartos Solteiros
            </DialogTitle>
            <p className="text-sm text-muted-foreground mt-2">
              Se houver apenas pessoas “single” (sem acompanhantes no contrato), selecione 2 ou 3 para
              criar um quarto (duplo/triplo solteiro) e salve para refletir no PDF Hotel.
            </p>
          </DialogHeader>

          <div className="space-y-4">
            {hotelGroupedFamiliesForUI.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium">Quartos já atribuídos</p>
                <div className="space-y-2">
                  {hotelGroupedFamiliesForUI.slice(0, 20).map((family: any, idx: number) => (
                    <div
                      key={`${family.key}-${idx}`}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <p className="text-xs text-muted-foreground">{(family.members || []).length} pessoa(s)</p>
                      </div>
                      <p className="mt-2 text-sm text-foreground/90">
                        {(family.members || [])
                          .map((m: any) => m.client_name)
                          .filter(Boolean)
                          .join(", ")}
                      </p>
                    </div>
                  ))}
                  {hotelGroupedFamiliesForUI.length > 20 && (
                    <p className="text-xs text-muted-foreground">Mostrando apenas os primeiros 20 grupos.</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium">Pessoas com quarto `SINGLE`</p>
              <p className="text-xs text-muted-foreground">
                Selecione 2 ou 3 para criar um quarto (duplo/triplo). Se selecionar 1, você pode adicionar essa pessoa em um quarto duplo existente.
              </p>

              {hotelSingleFamilies.length > 0 ? (
                <div className="space-y-2">
                  {hotelSingleFamilies.map((p: any) => {
                    const selected = hotelRoomSelection.some((s) => s.id === p.id);
                    return (
                      <button
                        key={p.id}
                        type="button"
                        className={[
                          "w-full text-left p-4 border rounded-2xl transition-all shadow-sm",
                          selected
                            ? "bg-green-50 border-green-500/60"
                            : "bg-white hover:bg-muted/40 hover:border-muted/60",
                        ].join(" ")}
                        onClick={() => toggleHotelRoomSelection(p)}
                        disabled={hotelRoomLinkLoading}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="font-medium">{p.client_name}</p>
                            <p className="text-xs text-muted-foreground">Assento: {p.seat_number}</p>
                          </div>
                ...
                <p className="text-sm text-muted-foreground">Nenhuma pessoa solteira para agrupar.</p>
              )}
            </div>

            {hotelRoomSelection.length === 1 && selectedHotelSingle && hotelDuploSolteiroRoomsForUI.length > 0 && (
              <div className="space-y-3 pt-1">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium">Adicionar ao quarto duplo existente</p>
                    <p className="text-xs text-muted-foreground">
                      Selecionado: <span className="font-medium">{selectedHotelSingle.client_name}</span>
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  {hotelDuploSolteiroRoomsForUI.slice(0, 10).map((room: any, idx: number) => {
                    const mainName = getFamilyMainName(room.members || []);
                    const roomMembersNames = (room.members || [])
                      .map((m: any) => m.client_name)
                      .filter(Boolean);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="font-medium">{mainName || "Quarto duplo"}</p>
                            <p className="text-xs text-muted-foreground">{roomMembersNames.join(", ")}</p>
                          </div>
                          <Button
                            variant="secondary"
                            className="rounded-full"
                            disabled={hotelRoomLinkLoading || !mainName}
                            onClick={() => handleAddSingleToDuploRoom(mainName)}
                          >
                            Adicionar
                          </Button>
                        </div>
                ...
                {hotelDuploSolteiroRoomsForUI.length > 10 && (
                  <p className="text-xs text-muted-foreground">Mostrando apenas os primeiros 10 quartos duplos.</p>
                )}
              </div>
            )}
          </div>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button
              variant="outline"
              onClick={handleContinueToHotelPDFReview}
              disabled={hotelRoomLinkLoading}
              data-testid="button-continue-hotel-pdf"
            >
              Continuar para gerar PDF
            </Button>

            {hotelRoomSelection.length === 2 && (
              <Button
                onClick={handleSaveHotelRoomsSelection}
                disabled={hotelRoomLinkLoading}
                data-testid="button-save-hotel-rooms-selection"
              >
                {hotelRoomLinkLoading ? "Salvando..." : "Salvar Duplo Solteiro"}
              </Button>
            )}
            {hotelRoomSelection.length === 3 && (
              <Button
                onClick={handleSaveHotelRoomsSelection}
                disabled={hotelRoomLinkLoading}
                data-testid="button-save-hotel-rooms-selection"
              >
                {hotelRoomLinkLoading ? "Salvando..." : "Salvar Triplo Solteiro"}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-3 max-h-[50vh] overflow-y-auto">
            {reservationsWithClients && reservationsWithClients.length > 0 ? (
              [...reservationsWithClients].sort((a: any, b: any) => {
                ...
                const getCpfRgString = () => {
                  if ((passenger.is_child || passenger.passenger_type === 'companion') && (passenger.child_data || passenger.child)) {
                    const child = passenger.child_data || passenger.child;
                    return child.cpf || child.rg;
                  }
                  return passenger.client?.cpf || passenger.client?.rg;
                };
                const cpfOrRg = getCpfRgString();
                ...
                return (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {cpfOrRg && (
                        <p className="text-xs text-muted-foreground">CPF/RG: {cpfOrRg}</p>
                      )}
