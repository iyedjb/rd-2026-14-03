...
const getDestinationName = (destination: Destination) => safeText(destination.name) || "Destino sem nome";
const getDestinationFileSlug = (destination: Destination) =>
  getDestinationName(destination).replace(/[^a-z0-9]/gi, '-');
...
export function DestinationList({ onEdit, onAdd, isVadmin }: DestinationListProps) {
  ...
  const generatePrevisaoPDF = (dest: Destination) => {
    ...
    doc.setFontSize(14);
    doc.setTextColor(40, 40, 40);
    doc.text(getDestinationName(dest), 15, 43);
    doc.setFontSize(10);
    const rows = [
    ...
    doc.save(`previsao-recebimento-${getDestinationFileSlug(dest)}-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  ...
  const generateFechamentoPDF = (dest: Destination) => {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(`Gerado em ${format(new Date(), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}`, 15, doc.internal.pageSize.getHeight() - 10);
    doc.save(`fechamento-${getDestinationFileSlug(dest)}-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  ...
  const { activeDestinations, archivedDestinations } = useMemo(() => {
    ...
    destinations.forEach((dest) => {
      ...
      if (endDate) {
        // Set end date to the end of the day (23:59:59) so it is not archived early
