...
export default function ClientHistory() {
  ...
  const filteredClients = useMemo(() => {
    ...
    const grouped = clients.reduce((acc, client) => {
      const nameKey = `${safeText(client.first_name).trim().toLowerCase()}-${safeText(client.last_name).trim().toLowerCase()}`;
      ...
      const key = safeText(client.cpf).trim() || `${nameKey}-${birthdateKey}`;
      ...
      const destinationInfo = {
        name: safeText(client.destination) || "Sem destino",
    ...
    return Object.values(grouped).filter(client => 
      `${safeText(client.first_name)} ${safeText(client.last_name)}`.toLowerCase().includes(searchTermLower) ||
      safeText(client.cpf).includes(searchTerm) ||
      safeText(client.phone).includes(searchTerm) ||
      safeText(client.email).toLowerCase().includes(searchTermLower) ||
      client.all_destinations.some((d: any) => safeText(d.name).toLowerCase().includes(searchTermLower))
    );
  }, [clients, searchTerm]);
