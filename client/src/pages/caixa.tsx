...
const getTransactionDescription = (transaction: Partial<FinancialTransaction>) =>
...
export default function Caixa() {
    ...
    try {
      ...
      if (reportFilterDestino !== "all" && receiptsData && receiptsData.length > 0) {
        ...
        filteredTransactions = filteredTransactions.filter(t => {
          const destId = descToDestino.get(getTransactionDescription(t));
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    ...
    let filtered = transactions.filter(transaction => {
      ...
      // Search filter
      const searchMatch = description.toLowerCase().includes(normalizedSearchTerm);
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "amount":
          ...
          break;
        case "description":
          result = getTransactionDescription(a).localeCompare(getTransactionDescription(b));
          break;
      }
  ...
  const totalFees = useMemo(() => {
    ...
    return filteredAndSortedTransactions.reduce((sum, t) => {
      ...
      const receipt = receiptsData.find((r: Receipt) => {
        ...
        return getTransactionDescription(t) === desc && t.payment_method === r.payment_method;
      });
