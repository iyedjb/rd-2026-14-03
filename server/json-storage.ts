...
export class JSONFileStorage implements IStorage {
  ...
  // PostgreSQL extension fields
  private usePostgres = false;
  public initializedPromise: Promise<void> = Promise.resolve();
  private isLoading = true;
  private pendingChanges = new Map<string, { type: 'upsert' | 'delete'; collection: string; id: string; data?: any }>();
  private rhPendingChanges = new Map<string, { type: 'upsert' | 'delete'; collection: string; id: string; data?: any }>();
  ...
  constructor() {
    ...
    if (process.env.DATABASE_URL) {
      ...
      console.log("[Postgres] DATABASE_URL found. Initializing PostgreSQL storage...");
      
      this.data = this.createEmptyData();
      this.rhData = this.createEmptyRHData();
      
      // Initialize with Proxy tracking wrapper
      this.data = this.createTrackingProxy(this.data, false);
      this.rhData = this.createTrackingProxy(this.rhData, true);
      
      this.initializedPromise = this.initPostgres();
    } else {
      console.log("[JSON] No DATABASE_URL found. Initializing JSON File storage...");
  ...
  private static readonly EAGER_COLLECTIONS = [
    'users',
    'clients',
    'prospects',
    'destinations',
  ...
  private async initPostgres() {
    try {
      console.log("[Postgres] Eagerly loading critical collections for startup...");
      await this.ensureCollectionsLoaded(JSONFileStorage.EAGER_COLLECTIONS);
      console.log("[Postgres] Critical collections loaded successfully!");
    } catch (error) {
      console.error("[Postgres] Eager load failed:", error);
    } finally {
      this.isLoading = false;
    }
  }

  public async ensureCollectionsLoaded(collections: string[]): Promise<void> {
    ...
    for (const col of collections) {
      ...
      let promise = this.loadingPromises.get(col);
      if (!promise) {
        promise = (async () => {
          const wasLoading = this.isLoading;
          this.isLoading = true;
          try {
            const isRh = col.startsWith('rh:');
            const dbCol = isRh ? col.substring(3) : col;
            const table = isRh ? 'rh_data' : 'app_data';

            console.log(`[Postgres] Lazily loading collection '${col}' from ${table}...`);
            const result = await pool.query(
              `SELECT collection, id, data FROM ${table} WHERE collection = $1`,
              [dbCol]
            );

            const rawTarget = isRh
              ? ((this.rhData as any).__target || this.rhData)
              : ((this.data as any).__target || this.data);

            if (!rawTarget[dbCol]) {
              rawTarget[dbCol] = {};
            }

            // Extract raw unproxied collection object to prevent triggering set traps
            const rawCollection = rawTarget[dbCol].__target || rawTarget[dbCol];

            for (const row of result.rows) {
              const { id, data: recordData } = row;
              rawCollection[id] = recordData;
            }

            this.loadedCollections.add(col);
            console.log(`[Postgres] Successfully loaded collection '${col}' (${result.rowCount} records)`);
          } catch (error) {
            console.error(`[Postgres] Failed to load collection '${col}':`, error);
            this.loadingPromises.delete(col);
            throw error;
          } finally {
        ...
        this.loadingPromises.set(col, promise);
      }
  ...
  public async ensureMethodDepsLoaded(methodName: string): Promise<void> {
    const deps = JSONFileStorage.METHOD_DEPS[methodName];
    if (deps && deps.length > 0) {
      await this.ensureCollectionsLoaded(deps);
    }
  }

  private static readonly METHOD_DEPS: Record<string, string[]> = {
    "getUser": ["users"],
    ...
    "deleteUser": ["users"],
    "getClient": ["clients"],
    "getClientByCpf": ["clients"],
    "getClients": ["clients"],
    "getAllClientsIncludingDeleted": ["clients"],
    "getClientsByDestination": ["clients"],
    "createClient": ["clients"],
    "updateClient": ["clients"],
    "deleteClient": ["clients"],
    "searchClients": ["clients"],
    "getClientByApprovalToken": ["clients"],
    "approveClient": ["clients"],
    "regenerateApprovalLink": ["clients"],
    "getReferralStatistics": ["clients"],
    "getDestination": ["destinations"],
    "getDestinations": ["destinations"],
    "getActiveDestinations": ["destinations"],
    "createDestination": ["destinations"],
    "updateDestination": ["destinations"],
    "deleteDestination": ["destinations"],
    "getChild": ["children"],
    "getChildrenByClientId": ["children", "clients"],
    "getAllChildren": ["children"],
    "createChild": ["children"],
    "updateChild": ["children"],
    "deleteChild": ["children"],
    "getMonthlyReport": ["monthly_reports"],
    "getMonthlyReports": ["monthly_reports"],
    "createMonthlyReport": ["monthly_reports"],
    "getFinancialTransaction": ["financial_transactions"],
    "getFinancialTransactions": ["financial_transactions"],
    "createFinancialTransaction": ["financial_transactions"],
    "updateFinancialTransaction": ["financial_transactions"],
    "deleteFinancialTransaction": ["financial_transactions"],
    "getProspect": ["prospects"],
    "getProspects": ["prospects"],
    "createProspect": ["prospects"],
    "updateProspect": ["prospects"],
    "deleteProspect": ["prospects"],
    "searchProspects": ["prospects"],
    "getProspectByQuoteToken": [],
    "updateQuoteStatus": [],
    "convertProspectToClient": ["prospects", "clients"],
    "getInactiveClient": ["inactive_clients"],
    "getInactiveClients": ["inactive_clients"],
    "createInactiveClient": ["inactive_clients"],
    "updateInactiveClient": ["inactive_clients"],
    "deleteInactiveClient": ["inactive_clients"],
    "searchInactiveClients": ["inactive_clients"],
    "getActivities": ["activities"],
    "createActivity": ["activities"],
    "getTopCreators": ["activities"],
    "getBus": ["buses"],
    "getBuses": ["buses"],
    "getActiveBuses": ["buses"],
    "createBus": ["buses"],
    "updateBus": ["buses"],
    "deleteBus": ["buses"],
    "getSeatReservation": ["seatReservations"],
    "getSeatReservations": ["seatReservations"],
    "getSeatReservationsByDestination": ["seatReservations"],
    "getSeatReservationsByBus": ["seatReservations"],
    "getSeatReservationsWithClientsByDestination": ["seatReservations", "clients", "children"],
    "getAllPassengersByDestination": ["clients", "destinations", "seatReservations", "children"],
    "createSeatReservation": ["seatReservations"],
    "updateSeatReservation": ["seatReservations"],
    "deleteSeatReservation": ["seatReservations"],
    "deleteSeatReservationsByChildId": ["seatReservations"],
    "deleteSeatReservationsByClientId": ["seatReservations"],
    "getSeatReservationByDestinationAndSeat": ["seatReservations", "clients"],
    "getReceipt": ["receipts"],
    ...
    "getParcelasByClientId": ["clients", "parcelas"],
    "getParcelasByMonth": ["parcelas"],
    "createParcela": ["parcelas"],
    "updateParcela": ["parcelas"],
    "deleteParcela": ["parcelas"],
    "deleteParcelasByClientId": ["clients", "parcelas"],
    "getDepartment": ["departments"],
    "getDepartments": ["departments"],
    "getActiveDepartments": ["departments"],
    "createDepartment": ["departments"],
    "updateDepartment": ["departments"],
    "deleteDepartment": ["departments"],
    "getTimeRecord": ["timeRecords"],
    "getTimeRecords": ["timeRecords"],
    "getTimeRecordsByUserId": ["timeRecords"],
    "getTimeRecordsByDateRange": ["timeRecords"],
    "getTodayTimeRecord": ["timeRecords"],
    "createTimeRecord": ["timeRecords"],
    "updateTimeRecord": ["timeRecords"],
    "deleteTimeRecord": ["timeRecords"],
    "getFacialVerificationSession": ["facialVerificationSessions"],
    "getFacialVerificationSessionByToken": ["facialVerificationSessions"],
    "createFacialVerificationSession": ["facialVerificationSessions"],
    "updateFacialVerificationSession": ["facialVerificationSessions"],
    "deleteFacialVerificationSession": ["facialVerificationSessions"],
    "cleanupExpiredSessions": ["facialVerificationSessions"],
    "getDiscountApprovalRequest": ["discountApprovalRequests"],
    "getDiscountApprovalRequests": ["discountApprovalRequests"],
    "getPendingDiscountApprovalRequests": ["discountApprovalRequests"],
    "createDiscountApprovalRequest": ["discountApprovalRequests"],
    "approveDiscountRequest": ["discountApprovalRequests"],
    "rejectDiscountRequest": ["discountApprovalRequests"],
    "deleteDiscountApprovalRequest": ["discountApprovalRequests"],
    "getInvitationLink": ["invitationLinks"],
    "createInvitationLink": ["invitationLinks"],
    "updateInvitationLinkUsage": ["invitationLinks"],
    "getCrmTask": ["crmTasks"],
    "getCrmTasks": ["crmTasks"],
    "getCrmTasksByUserId": ["crmTasks"],
    "getCrmTasksCreatedBy": ["crmTasks"],
    "createCrmTask": ["crmTasks"],
    "updateCrmTask": ["crmTasks"],
    "deleteCrmTask": ["crmTasks"],
    "getNotifications": ["notifications"],
    "createNotification": ["notifications"],
    "markNotificationAsRead": ["notifications"],
    "checkAndCreateBillReminders": ["bills", "notifications", "parcelas", "clients", "users"],
    "getFuncionario": ["rh:funcionarios"],
    "getFuncionarios": ["rh:funcionarios"],
    "getActiveFuncionarios": ["rh:funcionarios"],
    "getTrialFuncionarios": ["rh:funcionarios"],
    "createFuncionario": ["rh:funcionarios"],
    "createTrialFuncionario": ["rh:funcionarios"],
    "updateFuncionario": ["rh:funcionarios"],
    "activateTrialFuncionario": ["rh:funcionarios"],
    "terminateFuncionario": ["rh:funcionarios"],
    "deleteFuncionario": ["rh:funcionarios"],
    "getProposal": ["rh:proposals"],
    "getProposals": ["rh:proposals"],
    "createProposal": ["rh:proposals"],
    "deleteProposal": ["rh:proposals"],
    "getCancelledClientCredit": ["cancelledClientCredits"],
    "getCancelledClientCredits": ["cancelledClientCredits"],
    "getCancelledClientCreditsByClientId": ["cancelledClientCredits"],
    "getActiveCancelledClientCredits": ["cancelledClientCredits"],
    "getExpiredCancelledClientCredits": ["cancelledClientCredits"],
    "createCancelledClientCredit": ["cancelledClientCredits"],
    "updateCancelledClientCredit": ["cancelledClientCredits"],
    "markCreditAsUsed": ["cancelledClientCredits"],
    "updateExpiredCredits": ["cancelledClientCredits"],
    "cancelClient": ["clients", "receipts", "seatReservations", "cancelledClientCredits"],
    "getBill": ["bills"],
    "getBills": ["bills"],
    "getBillsByStatus": ["bills"],
    "createBill": ["bills"],
    "updateBill": ["bills"],
    "deleteBill": ["bills"]
  };
  ...
  private createEmptyRHData(): RHDatabaseData {
    return {
      funcionarios: {},
      proposals: {},
    };
  ...
  private wrappedCollections = new Set<string>();

  private createTrackingProxy(target: any, isRH: boolean, collectionName?: string): any {
    const self = this;
    if (collectionName) {
      // Collection-level proxy: intercepts mutations on individual records
      return new Proxy(target, {
        ...
        set(obj, prop, value) {
          ...
          if (self.usePostgres) {
            ...
            const changes = isRH ? self.rhPendingChanges : self.pendingChanges;
            changes.set(key, {
              type: 'upsert',
              collection: collectionName,
              id,
              data: self.convertDatesToTimestamp(value)
            });
            if (isRH) {
              self.saveRHData();
            } else {
      ...
      // Top-level proxy: lazily wraps each collection sub-object
      return new Proxy(target, {
        get(obj, prop) {
          if (prop === '__target') {
            return obj;
          }
          // Allow standard operations to pass through
          if (typeof prop === 'symbol') {
            return Reflect.get(obj, prop);
          }
          const collection = prop as string;
          const colObj = obj[collection];
          if (typeof colObj === 'object' && colObj !== null && !self.wrappedCollections.has((isRH ? 'rh:' : '') + collection)) {
            obj[collection] = self.createTrackingProxy(colObj, isRH, collection);
            self.wrappedCollections.add((isRH ? 'rh:' : '') + collection);
          }
  ...
  private loadRHData(): RHDatabaseData {
    const emptyData = this.createEmptyRHData();
  ...
  private saveData(): void {
    ...
    if (this.usePostgres) {
      this.saveDebounceTimer = setTimeout(async () => {
        ...
        const changesToFlush = Array.from(this.pendingChanges.values());
        ...
        try {
          for (const change of changesToFlush) {
            if (change.type === 'upsert') {
              await pool.query(
                `INSERT INTO app_data (collection, id, data) 
                 VALUES ($1, $2, $3) 
                 ON CONFLICT (collection, id) 
                 DO UPDATE SET data = $3`,
                [change.collection, change.id, JSON.stringify(change.data)]
              );
            } else if (change.type === 'delete') {
              await pool.query(
                `DELETE FROM app_data WHERE collection = $1 AND id = $2`,
                [change.collection, change.id]
              );
            }
          }
        } catch (error) {
          console.error("[Postgres] Error flushing app_data changes:", error);
        }
      }, 100);
    } else {
  ...
  private saveRHData(): void {
    ...
    if (this.usePostgres) {
      this.rhSaveDebounceTimer = setTimeout(async () => {
        if (this.rhPendingChanges.size === 0) return;
        
        const changesToFlush = Array.from(this.rhPendingChanges.values());
        this.rhPendingChanges.clear();
        
        try {
          for (const change of changesToFlush) {
            if (change.type === 'upsert') {
              await pool.query(
                `INSERT INTO rh_data (collection, id, data) 
                 VALUES ($1, $2, $3) 
                 ON CONFLICT (collection, id) 
                 DO UPDATE SET data = $3`,
                [change.collection, change.id, JSON.stringify(change.data)]
              ...
              await pool.query(
                `DELETE FROM rh_data WHERE collection = $1 AND id = $2`,
                [change.collection, change.id]
              );
            }
          }
        } catch (error) {
          console.error("[Postgres] Error flushing rh_data changes:", error);
        }
      }, 100);
    } else {
  ...
  private convertTimestampsToDates(obj: any): any {
    ...
    // Convert Firebase/Firestore timestamps to Date objects
    if (typeof obj === "object") {
  ...
  async getActiveDestinations(): Promise<Destination[]> {
    ...
    const localTodayStr = this.getLocalDateString(new Date()); // e.g. "2026-06-13"
    
    return destinations.filter((d) => {
      if (!d.is_active) return false;
      // Filter out destinations where the travel period has ended (in local Brazil date)
      if (d.periodo_viagem_fim) {
        let endDateStr = "";
        if (d.periodo_viagem_fim instanceof Date) {
          endDateStr = this.getLocalDateString(d.periodo_viagem_fim);
        } else if (typeof d.periodo_viagem_fim === "string") {
          endDateStr = String(d.periodo_viagem_fim).split('T')[0];
        }
        
        if (endDateStr && endDateStr < localTodayStr) {
          return false;
        }
      }
      return true;
  ...
  async getSeatReservationsWithClientsByDestination(
    ...
    const childrenByClientIdCache = new Map<string, Child[]>();
    ...
    for (const reservation of reservations) {
      ...
      if (reservation.child_id) {
        ...
        // Legacy fallback: older seat-selection flows stored acompanhantes in seat_reservations
        // without `child_id` (but put the child's name into `client_name`).
        // When that happens, PDFs would incorrectly fall back to the parent's CPF/RG/birthdate.
        ...
        const normalizedReservationName = normalizeName(reservation.client_name);

        let children = childrenByClientIdCache.get(reservation.client_id);
        if (!children) {
          children = await this.getChildrenByClientId(reservation.client_id);
          childrenByClientIdCache.set(reservation.client_id, children);
        }

        const matchingChild = normalizedReservationName
          ? (children || []).find(c => (c.name || "").trim().toUpperCase() === normalizedReservationName)
          : undefined;
  ...
  async getNotifications(userEmail: string, unreadOnly?: boolean): Promise<Notification[]> {
    ...
    // First filter raw data to avoid converting dates on thousands of unrelated notifications
    const matchingKeys = Object.keys(this.data.notifications).filter(id => {
      const n = this.data.notifications[id];
      if (!n) return false;
      const matchesEmail = n.user_email && n.user_email.toLowerCase() === userEmailLower;
      if (!matchesEmail) return false;
      if (unreadOnly && n.read) return false;
      return true;
    });

    const notifications = matchingKeys.map((id) => 
      this.convertTimestampsToDates({ id, ...this.data.notifications[id] })
    );
  ...
  async checkAndCreateBillReminders(): Promise<void> {
    ...
    // Build a lookup set of existing notifications of interest to avoid N*M database/memory queries
    const existingReminders = new Set<string>();
    for (const [id, n] of Object.entries(this.data.notifications)) {
      const type = (n as any).type;
      if (type === "payment_reminder" || type === "parcela_overdue") {
        existingReminders.add(`${(n as any).user_email}:${type}:${(n as any).related_id}`);
      }
    }
    ...
    for (const bill of bills) {
      if (bill.status === "pending") {
        ...
        if (billDate > now && billDate <= fiveDaysFromNow) {
          const key = `${bill.created_by_email}:payment_reminder:${bill.id}`;
    ...
    // Map clients by ID for O(1) fast lookup
