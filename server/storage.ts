...
// Use JSON File Storage with PostgreSQL support
import { JSONFileStorage } from "./json-storage";

console.log("Initializing database storage...");
const rawStorage = new JSONFileStorage();

// Wrap in a Proxy to transparently await PostgreSQL loading and lazily load required collections
const storage: IStorage = new Proxy(rawStorage, {
  get(target, prop, receiver) {
    const value = Reflect.get(target, prop, receiver);
    if (typeof value === 'function' && prop !== 'createTrackingProxy' && prop !== 'initPostgres' && typeof prop === 'string') {
      const methodName = prop;
      return async function (...args: any[]) {
        await (target as any).initializedPromise;
        if (typeof (target as any).ensureMethodDepsLoaded === 'function') {
          await (target as any).ensureMethodDepsLoaded(methodName);
        }
...
if (process.env.DATABASE_URL) {
  console.log("[Postgres] Database Storage initialized successfully (main + RH)");
} else {
  console.log("[JSON] Database Storage initialized successfully (main + RH)");
}
